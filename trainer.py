...
from utils import evidential_loss, EvidentialLossNLL
...
class Trainer(object):
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            train_lst = ["epoch " + str(self.current_epoch)] + list(map(float2str, [train_loss,mse,mae,rmse,pearson_corr,r2]))
            ...
            val_lst = ["epoch " + str(self.current_epoch)] + list(map(float2str, [val_loss,mse,mae,rmse,pearson_corr,r2]))
        ...
        test_lst = ["epoch " + str(self.best_epoch)] + list(map(float2str, [val_loss,mse,mae,rmse,pearson_corr,r2]))
    ...
    def train_epoch(self):
        ...
        for i, (v_d, v_p, labels,weights) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            # 兼容点云解包 (MolT5, MACCS, PC, FragmentGraph)
            v_d = (v_d[0].to(self.device), v_d[1].to(self.device), v_d[2].to(self.device), v_d[3].to(self.device))
            ...
            v_d_out, v_p_out, f, score = self.model(v_d, v_p, mode="train")
            ...
            batch_preds = score.data.cpu().numpy().tolist()
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            ...
            for i, (v_d, v_p, labels,weights) in enumerate(data_loader):
                ...
                if dataloader == "val":
                    ...
                    v_d_out, v_p_out, f, score = self.best_model(v_d, v_p, mode="train")
        ...
        p, y, c, var = [], [], [], []
        data_uncertainty_list, model_uncertainty_list, total_uncertainty_list = [], [], []
